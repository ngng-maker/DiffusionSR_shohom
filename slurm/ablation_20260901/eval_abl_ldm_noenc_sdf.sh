#!/bin/bash
#SBATCH --job-name=eval_ldm_noenc_sdf
...
#SBATCH --mem=32G
#SBATCH --output=logs/ablation_20260901/eval_ldm_noenc_sdf_%j.log
#SBATCH --error=logs/ablation_20260901/eval_ldm_noenc_sdf_%j.err
#SBATCH --requeue
...
python -m diffusionsr.analysis.eval_predictions \
    --run_name        abl_ldm_noenc_sdf \
    --wandb_run_name  1_Sep_2026_ldm_noenc_sdf \
    --model_type      ldm \
    --model_dir       "$RUNS/abl_ldm_noenc_sdf" \
    --vae_dir         "$RUNS/vae_sdf" \
    --data_root       "$DATA" \
    --field_names     temperature liqlabel \
    --batch_size      2

echo "=== eval_abl_ldm_noenc_sdf complete ==="

#!/bin/bash
#SBATCH --job-name=eval_fm_enc_temp
...
#SBATCH --mem=32G
#SBATCH --output=logs/ablation_20260901/eval_fm_enc_temp_%j.log
#SBATCH --error=logs/ablation_20260901/eval_fm_enc_temp_%j.err
#SBATCH --requeue
...
python -m diffusionsr.analysis.eval_predictions \
    ...
    --wandb_run_name  1_Sep_2026_fm_enc_temp \
    --model_type      flow_matching \
    --model_dir       "$RUNS/abl_fm_enc_temp" \
    --enc_dir         "$RUNS/enc_temp" \
    --data_root       "$DATA" \
    --field_names     temperature \
    --batch_size      4

echo "=== eval_abl_fm_enc_temp complete ==="

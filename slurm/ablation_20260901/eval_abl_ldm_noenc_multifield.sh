#!/bin/bash
#SBATCH --job-name=eval_ldm_noenc_multifield
...
#SBATCH --mem=32G
#SBATCH --output=logs/ablation_20260901/eval_ldm_noenc_multifield_%j.log
#SBATCH --error=logs/ablation_20260901/eval_ldm_noenc_multifield_%j.err
#SBATCH --requeue
set -eo pipefail
...
# TODO: fill in after training starts — e.g. $(date +"%d_%b_%Y")_ldm_noenc_multifield
WANDB_RUN_NAME=""
...
python -m diffusionsr.analysis.eval_predictions \
    --run_name        abl_ldm_noenc_multifield \
    --wandb_run_name  "$WANDB_RUN_NAME" \
    --model_type      ldm \
    --model_dir       "$RUNS/abl_ldm_noenc_multifield" \
    --vae_dir         "$RUNS/vae_multifield" \
    --data_root       "$DATA" \
    --field_names     temperature sdfliqlabel \
    --batch_size      4

echo "=== eval_abl_ldm_noenc_multifield complete ==="

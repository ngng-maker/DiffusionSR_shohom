#!/bin/bash
#SBATCH --job-name=eval_ldm_noenc_temp
...
#SBATCH --mem=32G
#SBATCH --output=logs/ablation_20260901/eval_ldm_noenc_temp_%j.log
#SBATCH --error=logs/ablation_20260901/eval_ldm_noenc_temp_%j.err
#SBATCH --requeue
...
python -m diffusionsr.analysis.eval_predictions \
    ...
    --model_type      ldm \
    --model_dir       "$RUNS/abl_ldm_noenc_temp" \
    --vae_dir         "$RUNS/vae_temp" \
    --data_root       "$DATA" \
    --field_names     temperature \
    --batch_size      2

echo "=== eval_abl_ldm_noenc_temp complete ==="

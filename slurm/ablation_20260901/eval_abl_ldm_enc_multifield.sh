#!/bin/bash
#SBATCH --job-name=eval_ldm_enc_multifield
...
#SBATCH --mem=32G
#SBATCH --output=logs/ablation_20260901/eval_ldm_enc_multifield_%j.log
#SBATCH --error=logs/ablation_20260901/eval_ldm_enc_multifield_%j.err
#SBATCH --requeue
set -eo pipefail
...
# TODO: fill in after training starts — e.g. $(date +"%d_%b_%Y")_ldm_enc_multifield
WANDB_RUN_NAME=""
...
python -m diffusionsr.analysis.eval_predictions \
    --run_name        abl_ldm_enc_multifield \
    --wandb_run_name  "$WANDB_RUN_NAME" \
    --model_type      ldm \
    --model_dir       "$RUNS/abl_ldm_enc_multifield" \
...
echo "=== eval_abl_ldm_enc_multifield complete ==="

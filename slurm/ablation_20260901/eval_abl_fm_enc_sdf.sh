#!/bin/bash
#SBATCH --job-name=eval_fm_enc_sdf
...
#SBATCH --mem=32G
#SBATCH --output=logs/ablation_20260901/eval_fm_enc_sdf_%j.log
#SBATCH --error=logs/ablation_20260901/eval_fm_enc_sdf_%j.err
#SBATCH --requeue
...
python -m diffusionsr.analysis.eval_predictions \
    ...
    --model_dir       "$RUNS/abl_fm_enc_sdf" \
    --enc_dir         "$RUNS/enc_sdf" \
    --data_root       "$DATA" \
    --field_names     temperature liqlabel \
    --batch_size      4

echo "=== eval_abl_fm_enc_sdf complete ==="

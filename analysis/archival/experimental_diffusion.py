...
class DiffusionModel():
    ...
    def save(self, split, all_images, epoch, step, res, hr, true_lr, upscaled_lr, plotting_field = 'temperature' ):
        ...
        if dataset.normalize == 'standardize':
            result = dataset.unscale_data(all_images.numpy()[-1,0,0], input_type = 'hr')
            target = dataset.unscale_data(hr[0,0].cpu().numpy(), input_type = 'hr')
            lr_plot = dataset.unscale_data(true_lr[0,0].cpu().numpy(), input_type = 'lr')
            upscaled_lr_plot = dataset.unscale_data(upscaled_lr[0,0].cpu().numpy(), input_type = 'lr')



        else:
            result = all_images.numpy()[-1,0,0]
            target = hr[0,0].cpu().numpy()
            lr_plot = true_lr[0,0].cpu().numpy()
            upscaled_lr_plot = upscaled_lr[0,0].cpu().numpy()
        plt.imshow(result, origin = 'lower', cmap = 'jet', vmin = 0, vmax = 1)
        ...
        plt.imshow(target, origin = 'lower', cmap = 'jet', vmin = 0, vmax = 1)
        ...
        plt.clf()
        plt.imshow(upscaled_lr_plot/255, origin = 'lower', cmap = 'jet', vmin = 0, vmax = 1)
        ...
        plt.imshow(lr_plot/255, origin = 'lower', cmap = 'jet', vmin = 0, vmax = 1)
    ...
    def train(self,
        ...
        for epoch in tqdm(range(self.start_epoch, self.epochs)):
            losses = []
            self.model.train()
            res, hr, true_lr, upscaled_lr = iter(self.train_loader).next()
            # breakpoint()
            batch = hr
            if self.encoding:
            ...
            for step, (res, hr, true_lr, upscaled_lr) in tqdm(enumerate(self.train_loader), total = len(self.train_loader)):
                ...
                # print(step, len(self.train_loader), hr.shape, true_lr.shape, upscaled_lr.shape)
                batch = hr

...
for encoding, condition in zip(['False', 'True', 'False', 'True'], ['implicit', 'explicit', 'explicit', 'implicit']):
    ...
    if encoding == 'True':
        ...
        case['encode_flag'] = 'upscaled'
    cases.append(case)
def process(case, limit =None, skip = 50, gpu  = 1, run_skip_ablation = False):
    ...
    batch_size = 16
    analysis_folder = f'/home/oogoke/DiffusionSR/runs/analysis/analyzed_figures_v2/{timesteps}_{conditioning}_{schedule}_{downscale_method}_{encode_flag}'
    ...
    def predict_refactored_diffusion(diff_model, lr_enc, res, hr, lr, upscaled_lr, dataset, skip = 50, sampler = 'DDIM'):
        ...
        all_images = diff_model.batch_sample(dataset = train_dataset, batch = hr, x_e = x_e, sampler = sampler, skip= skip)                
...
for i, case in enumerate(cases):
    ...
    # process(case, limit = None, skip = 50, gpu = i + 3, run_skip_ablation = True)
# i = 0

    p = multiprocessing.Process(target = process, args = (case,None, 500, i, False))
    p.start()
p.join()
